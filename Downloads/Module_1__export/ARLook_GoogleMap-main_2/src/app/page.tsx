...
import { sampleProperties, presetPrompts, filterPropertiesByMessage } from '@/shared/constants/properties';
  ...
  const handleSendMessage = useCallback(async (content: string) => {
    ...
    try {
      ...
      if (error?.message?.includes('Failed to process message')) {
        const response = error.response || await fetch('/api/chat', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ message: content, conversationHistory: updatedMessages }),
        }).catch(() => null);
        ...
        if (response) {
          try {
            ...
            errorMessage += `Server returned status ${response.status}. Please check if OpenAI API key is configured correctly.`;
          }
        } else {
