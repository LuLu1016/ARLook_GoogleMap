'use client';

import { useEffect, useRef, useState } from 'react';
import { Loader } from '@googlemaps/js-api-loader';
import { Property } from '@/shared/types';
import {
  createMapLoader,
  defaultMapConfig,
  getGoogleMapsApiKey,
  createMapOptions,
} from '@/client/utils/google-maps';

interface MapContainerProps {
  markers: Property[];
  center?: { lat: number; lng: number };
  zoom?: number;
  filters?: {
    maxPrice?: number;
    minPrice?: number;
    highlightedIds?: string[];
    amenities?: string[];
  };
  onFiltersUpdate?: (filters: MapContainerProps['filters'] | undefined) => void;
}

// Marker style definitions
const markerStyles = {
  default: { color: '#4285F4', label: 'P' }, // Blue - default
  inBudget: { color: '#34A853', label: '$' }, // Green - within budget
  highlighted: { color: '#EA4335', label: '!' }, // Red - recommended
  outOfRange: { color: '#9AA0A6', label: '×' }, // Gray - doesn't match
};

/**
 * Check if property matches filters
 * Returns true if no filters are set (show all by default)
 */
function isPropertyMatching(property: Property, filters?: MapContainerProps['filters']): boolean {
  // If no filters are set, show all properties
  if (!filters || (!filters.maxPrice && !filters.minPrice && !filters.amenities?.length)) {
    return true;
  }
  
  if (filters.maxPrice && property.price > filters.maxPrice) {
    return false;
  }
  if (filters.minPrice && property.price < filters.minPrice) {
    return false;
  }
  
  // Amenities filter (if set)
  if (filters.amenities && filters.amenities.length > 0) {
    const matchesAllAmenities = filters.amenities.every(amenity =>
      property.amenities.some(a => a.toLowerCase().includes(amenity.toLowerCase()))
    );
    if (!matchesAllAmenities) {
      return false;
    }
  }
  
  return true;
}

/**
 * Create smart marker style based on property and filters
 */
function getMarkerStyle(
  property: Property,
  filters?: MapContainerProps['filters']
): typeof markerStyles.default {
  // AI recommended properties
  if (filters?.highlightedIds?.includes(property.id)) {
    return markerStyles.highlighted;
  }
  
  // Budget matching
  if (filters?.maxPrice && property.price <= filters.maxPrice) {
    return markerStyles.inBudget;
  }
  
  // Doesn't match filters
  if (!isPropertyMatching(property, filters)) {
    return markerStyles.outOfRange;
  }
  
  return markerStyles.default;
}

/**
 * Create enhanced info window content - Elegant design
 */
function createInfoWindowContent(property: Property): string {
  const walkingInfo = property.walkingDistanceToWharton
    ? `<div style="display: flex; align-items: center; gap: 6px; margin-bottom: 12px; padding: 6px 10px; background: linear-gradient(135deg, rgba(251,191,36,0.1) 0%, rgba(245,158,11,0.1) 100%); border-radius: 8px; border: 1px solid rgba(251,191,36,0.2);">
        <span style="font-size: 14px;">🚶</span>
        <span style="font-size: 12px; color: #78716c; font-weight: 300;">${property.walkingDistanceToWharton} min walk to Wharton</span>
      </div>`
    : '';

  const amenitiesHtml = property.amenities.slice(0, 4).map(
    (amenity) => `<span style="
      background: linear-gradient(135deg, rgba(251,191,36,0.15) 0%, rgba(245,158,11,0.15) 100%);
      color: #92400e;
      font-size: 11px;
      padding: 4px 10px;
      border-radius: 12px;
      font-weight: 300;
      border: 1px solid rgba(251,191,36,0.3);
      white-space: nowrap;
    ">${amenity}</span>`
  ).join('');

  return `
    <div style="
      padding: 20px; 
      max-width: 320px; 
      font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;
      background: linear-gradient(135deg, rgba(255,255,255,0.98) 0%, rgba(250,250,250,0.98) 100%);
    ">
      <h3 style="
        font-weight: 400; 
        font-size: 20px; 
        margin-bottom: 6px; 
        color: #292524;
        letter-spacing: -0.02em;
        line-height: 1.3;
      ">${property.name}</h3>
      <p style="
        color: #78716c; 
        font-size: 13px; 
        margin-bottom: 14px;
        font-weight: 300;
        line-height: 1.4;
      ">${property.address}</p>
      <div style="
        display: flex; 
        align-items: baseline; 
        gap: 8px; 
        margin-bottom: 12px;
        padding-bottom: 12px;
        border-bottom: 1px solid rgba(0,0,0,0.08);
      ">
        <span style="
          font-size: 28px; 
          font-weight: 300; 
          color: #f59e0b;
          letter-spacing: -0.03em;
        ">$${property.price}</span>
        <span style="
          font-size: 12px; 
          color: #78716c; 
          font-weight: 300;
        ">per person</span>
        <span style="
          font-size: 13px; 
          color: #78716c; 
          font-weight: 300;
          margin-left: auto;
        ">${property.bedrooms}b${property.bathrooms}b</span>
      </div>
      ${walkingInfo}
      <div style="
        display: flex; 
        flex-wrap: wrap; 
        gap: 6px; 
        margin-bottom: 14px;
      ">
        ${amenitiesHtml}
      </div>
      <p style="
        font-size: 13px; 
        color: #57534e; 
        line-height: 1.6;
        font-weight: 300;
        font-style: italic;
      ">${property.description}</p>
    </div>
  `;
}

/**
 * Render walking time circles (isochrones)
 */
function renderWalkingCircles(
  map: google.maps.Map,
  center: { lat: number; lng: number }
): google.maps.Circle[] {
  const circles: google.maps.Circle[] = [];
  
  // Walking speed: ~4.5 km/h = ~75 m/min
  // So 10 min = ~750m, 20 min = ~1500m, 30 min = ~2250m
  const walkingCircles = [
    { radius: 800, color: '#34A853', label: '10 min' }, // ~10 min walk - green
    { radius: 1600, color: '#FBBC05', label: '20 min' }, // ~20 min walk - yellow
    { radius: 2400, color: '#EA4335', label: '30 min' }, // ~30 min walk - red
  ];
  
  walkingCircles.forEach((circle) => {
    const circleObj = new google.maps.Circle({
      map,
      center,
      radius: circle.radius,
      fillColor: circle.color,
      fillOpacity: 0.15,
      strokeWeight: 2,
      strokeColor: circle.color,
      strokeOpacity: 0.4,
      zIndex: 0, // Behind markers
    });
    
    circles.push(circleObj);
  });
  
  return circles;
}

export default function MapContainer({
  markers,
  center = defaultMapConfig.center,
  zoom = defaultMapConfig.zoom,
  filters,
  onFiltersUpdate,
}: MapContainerProps) {
  const mapRef = useRef<HTMLDivElement>(null);
  const [map, setMap] = useState<google.maps.Map | null>(null);
  const mapMarkersRef = useRef<google.maps.Marker[]>([]);
  const walkingCirclesRef = useRef<google.maps.Circle[]>([]);
  const [mapError, setMapError] = useState<string | null>(null);

  useEffect(() => {
    // Ensure this only runs on client-side
    if (typeof window === 'undefined') return;
    
    const initMap = async () => {
      const apiKey = getGoogleMapsApiKey();
      
      // Debug: Log API key status
      console.log('🗺️ Google Maps initialization:', {
        apiKey: apiKey ? `${apiKey.substring(0, 10)}...` : 'Not found',
        apiKeyLength: apiKey?.length || 0,
        hasMapRef: !!mapRef.current,
        mapAlreadyInitialized: !!map,
        envVar: process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY ? 'Found' : 'Not found',
      });
      
      // Check if API key is valid
      if (!apiKey || apiKey === 'your_api_key_here' || apiKey === 'your_google_maps_api_key_here') {
        console.error('❌ Google Maps API Key not configured');
        setMapError('Google Maps API Key not configured. Please check .env.local file.');
        return;
      }

      // Wait for mapRef to be available
      if (!mapRef.current) {
        console.warn('⚠️ Map ref not ready, retrying...');
        setTimeout(initMap, 100);
        return;
      }

      const loader = createMapLoader(apiKey);

      try {
        console.log('📦 Loading Google Maps library...');
        const { Map } = await loader.importLibrary('maps');
        await loader.importLibrary('marker');
        console.log('✅ Google Maps library loaded successfully');

        if (mapRef.current && !map) {
          console.log('🗺️ Initializing map instance...', { center, zoom });
          const mapInstance = new Map(mapRef.current, createMapOptions({ center, zoom }));

          setMap(mapInstance);
          setMapError(null);
          console.log('✅ Google Maps initialized successfully!', {
            center: mapInstance.getCenter()?.toJSON(),
            zoom: mapInstance.getZoom(),
          });
          
          // Render walking circles centered on Huntsman Hall immediately after map loads
          const huntsmanHall = { lat: 39.9546, lng: -75.1960 };
          const initialCircles = renderWalkingCircles(mapInstance, huntsmanHall);
          walkingCirclesRef.current = initialCircles;
          console.log('✅ Walking circles rendered:', { count: initialCircles.length });
        } else {
          console.warn('⚠️ Map container not ready or map already initialized', {
            hasMapRef: !!mapRef.current,
            mapExists: !!map,
          });
        }
      } catch (error: any) {
        console.error('❌ Error loading Google Maps:', error);
        console.error('❌ Error details:', {
          name: error?.name,
          message: error?.message,
          stack: error?.stack,
          toString: error?.toString(),
        });
        
        let errorMessage = 'Failed to load Google Maps. ';
        let fixInstructions = '';
        
        const errorName = error?.name || '';
        const errorMsg = error?.message || '';
        const combinedError = `${errorName} ${errorMsg}`.toLowerCase();
        
        if (combinedError.includes('referernotallowed') || combinedError.includes('referer')) {
          errorMessage += 'API Key restriction not configured correctly.';
          fixInstructions = '\n\nFix: Go to Google Cloud Console → Credentials → Edit API Key → Application restrictions → HTTP referrers → Add: http://localhost:3000/*';
        } else if (combinedError.includes('invalidkey') || combinedError.includes('invalid')) {
          errorMessage += 'API Key is invalid or Maps JavaScript API is not enabled.';
          fixInstructions = '\n\nFix: 1) Enable Maps JavaScript API in Google Cloud Console → APIs & Services → Library → Search "Maps JavaScript API" → Enable\n2) Check API Key restrictions allow "Maps JavaScript API"\n3) Verify API key in .env.local is correct';
        } else if (combinedError.includes('apinotactivated') || combinedError.includes('not activated')) {
          errorMessage += 'Maps JavaScript API is not enabled.';
          fixInstructions = '\n\nFix: Go to Google Cloud Console → APIs & Services → Library → Search "Maps JavaScript API" → Enable';
        } else if (combinedError.includes('missingkey') || combinedError.includes('missing')) {
          errorMessage += 'Google Maps API Key is missing.';
          fixInstructions = '\n\nFix: 1) Check .env.local file exists in src/ directory\n2) Ensure NEXT_PUBLIC_GOOGLE_MAPS_API_KEY is set\n3) Restart dev server (npm run dev)';
        } else {
          errorMessage += `Error: ${error?.message || error?.name || 'Unknown error'}`;
          fixInstructions = '\n\nFix: Check browser console (F12) for detailed error message. See README.md Troubleshooting section.';
        }
        
        setMapError(errorMessage + fixInstructions);
      }
    };

    initMap();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Add map controls (price slider, amenities filter) - Elegant styled controls
  useEffect(() => {
    if (!map || !onFiltersUpdate) return;

    // Price control - Elegant design matching sidebar
    const priceControlDiv = document.createElement('div');
    priceControlDiv.style.cssText = 'margin: 12px; padding: 0;';
    priceControlDiv.innerHTML = `
      <div style="
        background: linear-gradient(135deg, rgba(255,255,255,0.95) 0%, rgba(250,250,250,0.95) 100%);
        backdrop-filter: blur(12px);
        padding: 16px;
        border-radius: 16px;
        box-shadow: 0 4px 20px rgba(0,0,0,0.08), 0 0 0 1px rgba(0,0,0,0.04);
        font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;
        min-width: 240px;
        transition: all 0.3s ease;
      ">
        <label style="display: block; font-size: 12px; font-weight: 500; margin-bottom: 10px; color: #78716c; text-transform: uppercase; letter-spacing: 0.05em;">
          Max Price
        </label>
        <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px;">
          <span style="font-size: 24px; font-weight: 300; color: #292524; letter-spacing: -0.02em;">$<span id="priceValue">${filters?.maxPrice || 3000}</span></span>
          <span style="font-size: 12px; color: #78716c; font-weight: 300;">per person</span>
        </div>
        <input 
          type="range" 
          min="1000" 
          max="5000" 
          step="100" 
          value="${filters?.maxPrice || 3000}" 
          id="priceSlider"
          style="
            width: 100%; 
            height: 6px;
            border-radius: 3px;
            background: linear-gradient(to right, #fbbf24 0%, #f59e0b 50%, #d97706 100%);
            outline: none;
            cursor: pointer;
            -webkit-appearance: none;
          "
        />
        <style>
          #priceSlider::-webkit-slider-thumb {
            -webkit-appearance: none;
            appearance: none;
            width: 18px;
            height: 18px;
            border-radius: 50%;
            background: linear-gradient(135deg, #fbbf24 0%, #f59e0b 100%);
            cursor: pointer;
            box-shadow: 0 2px 8px rgba(251, 191, 36, 0.4);
            border: 2px solid white;
            transition: all 0.2s ease;
          }
          #priceSlider::-webkit-slider-thumb:hover {
            transform: scale(1.15);
            box-shadow: 0 4px 12px rgba(251, 191, 36, 0.6);
          }
          #priceSlider::-moz-range-thumb {
            width: 18px;
            height: 18px;
            border-radius: 50%;
            background: linear-gradient(135deg, #fbbf24 0%, #f59e0b 100%);
            cursor: pointer;
            box-shadow: 0 2px 8px rgba(251, 191, 36, 0.4);
            border: 2px solid white;
          }
        </style>
      </div>
    `;
    
    map.controls[google.maps.ControlPosition.TOP_LEFT].push(priceControlDiv);

    // Amenities control - Elegant design
    const amenitiesControlDiv = document.createElement('div');
    amenitiesControlDiv.style.cssText = 'margin: 12px; padding: 0;';
    amenitiesControlDiv.innerHTML = `
      <div style="
        background: linear-gradient(135deg, rgba(255,255,255,0.95) 0%, rgba(250,250,250,0.95) 100%);
        backdrop-filter: blur(12px);
        padding: 16px;
        border-radius: 16px;
        box-shadow: 0 4px 20px rgba(0,0,0,0.08), 0 0 0 1px rgba(0,0,0,0.04);
        font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;
        min-width: 200px;
      ">
        <div style="font-size: 12px; font-weight: 500; margin-bottom: 12px; color: #78716c; text-transform: uppercase; letter-spacing: 0.05em;">
          Amenities
        </div>
        <div style="display: flex; flex-direction: column; gap: 10px;">
          ${['Gym', 'In-unit laundry', 'Parking', 'Pool'].map(amenity => {
            const isChecked = filters?.amenities?.some(a => a.toLowerCase().includes(amenity.toLowerCase())) || false;
            return `
            <label style="
              display: flex; 
              align-items: center; 
              font-size: 13px; 
              cursor: pointer;
              padding: 6px 8px;
              border-radius: 8px;
              transition: all 0.2s ease;
              color: #292524;
              font-weight: 300;
            " onmouseover="this.style.background='rgba(251,191,36,0.1)'" onmouseout="this.style.background='transparent'">
              <input 
                type="checkbox" 
                class="amenity-checkbox" 
                value="${amenity}" 
                ${isChecked ? 'checked' : ''} 
                style="
                  margin-right: 10px; 
                  cursor: pointer;
                  width: 16px;
                  height: 16px;
                  accent-color: #f59e0b;
                "
              />
              <span>${amenity}</span>
            </label>
          `;
          }).join('')}
        </div>
      </div>
    `;
    
    map.controls[google.maps.ControlPosition.TOP_RIGHT].push(amenitiesControlDiv);

    // Add walking time legend - Elegant design
    const legendDiv = document.createElement('div');
    legendDiv.style.cssText = 'margin: 12px; padding: 0;';
    legendDiv.innerHTML = `
      <div style="
        background: linear-gradient(135deg, rgba(255,255,255,0.95) 0%, rgba(250,250,250,0.95) 100%);
        backdrop-filter: blur(12px);
        padding: 16px;
        border-radius: 16px;
        box-shadow: 0 4px 20px rgba(0,0,0,0.08), 0 0 0 1px rgba(0,0,0,0.04);
        font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;
        min-width: 200px;
      ">
        <div style="font-size: 12px; font-weight: 500; margin-bottom: 12px; color: #78716c; text-transform: uppercase; letter-spacing: 0.05em;">
          Walking Time
        </div>
        <div style="display: flex; flex-direction: column; gap: 10px;">
          <div style="display: flex; align-items: center; gap: 10px;">
            <div style="width: 14px; height: 14px; border-radius: 50%; background: #34A853; border: 2px solid #34A853; box-shadow: 0 2px 4px rgba(52,168,83,0.3);"></div>
            <span style="font-size: 13px; color: #292524; font-weight: 300;">10 min walk</span>
          </div>
          <div style="display: flex; align-items: center; gap: 10px;">
            <div style="width: 14px; height: 14px; border-radius: 50%; background: #FBBC05; border: 2px solid #FBBC05; box-shadow: 0 2px 4px rgba(251,188,5,0.3);"></div>
            <span style="font-size: 13px; color: #292524; font-weight: 300;">20 min walk</span>
          </div>
          <div style="display: flex; align-items: center; gap: 10px;">
            <div style="width: 14px; height: 14px; border-radius: 50%; background: #EA4335; border: 2px solid #EA4335; box-shadow: 0 2px 4px rgba(234,67,53,0.3);"></div>
            <span style="font-size: 13px; color: #292524; font-weight: 300;">30 min walk</span>
          </div>
        </div>
        <div style="margin-top: 12px; padding-top: 12px; border-top: 1px solid rgba(0,0,0,0.08);">
          <div style="font-size: 11px; color: #78716c; line-height: 1.5; font-weight: 300; font-style: italic;">
            From Wharton School
          </div>
        </div>
      </div>
    `;
    
    map.controls[google.maps.ControlPosition.BOTTOM_LEFT].push(legendDiv);

    // Setup event listeners
    const priceSlider = document.getElementById('priceSlider') as HTMLInputElement;
    const priceValue = document.getElementById('priceValue');
    
    if (priceSlider && priceValue) {
      priceSlider.addEventListener('input', (e) => {
        const price = parseInt((e.target as HTMLInputElement).value);
        priceValue.textContent = price.toString();
        onFiltersUpdate({
          ...filters,
          maxPrice: price,
        });
      });
    }

    // Amenities checkboxes - use event delegation to handle dynamically created checkboxes
    amenitiesControlDiv.addEventListener('change', (e) => {
      const target = e.target as HTMLInputElement;
      if (target.classList.contains('amenity-checkbox')) {
        const amenityCheckboxes = amenitiesControlDiv.querySelectorAll('.amenity-checkbox');
        const checkedAmenities = Array.from(amenityCheckboxes)
          .filter((cb: any) => cb.checked)
          .map((cb: any) => cb.value);
        
        onFiltersUpdate({
          ...filters,
          amenities: checkedAmenities.length > 0 ? checkedAmenities : undefined,
        });
      }
    });

      // Cleanup function
      return () => {
        map.controls[google.maps.ControlPosition.TOP_LEFT].clear();
        map.controls[google.maps.ControlPosition.TOP_RIGHT].clear();
        map.controls[google.maps.ControlPosition.BOTTOM_LEFT].clear();
      };
  }, [map, filters, onFiltersUpdate]);

  // Render walking circles centered on Huntsman Hall (always, regardless of map center)
  useEffect(() => {
    if (!map) return;
    
    // Clear existing circles
    walkingCirclesRef.current.forEach((circle) => {
      circle.setMap(null);
    });
    walkingCirclesRef.current = [];
    
    // Always use Huntsman Hall coordinates for walking circles
    const huntsmanHall = { lat: 39.9546, lng: -75.1960 };
    const circles = renderWalkingCircles(map, huntsmanHall);
    walkingCirclesRef.current = circles;
    
    console.log('📍 Walking circles rendered centered on Huntsman Hall:', huntsmanHall);
  }, [map]);

  // Update map center when center prop changes (but keep circles on Huntsman Hall)
  useEffect(() => {
    if (map && center) {
      map.setCenter(center);
    }
  }, [map, center]);

  // Update markers when markers prop changes
  useEffect(() => {
    if (!map) {
      console.log('⏳ Waiting for map initialization...');
      return;
    }

    console.log('📍 Updating map markers:', {
      markerCount: markers.length,
      filters,
      properties: markers.map(p => ({ 
        name: p.name, 
        price: p.price, 
        lat: p.latitude, 
        lng: p.longitude,
        matchesMaxPrice: filters?.maxPrice ? p.price <= filters.maxPrice : 'N/A',
        matchesFilters: isPropertyMatching(p, filters)
      }))
    });

    // Clear existing markers
    mapMarkersRef.current.forEach((marker) => {
      marker.setMap(null);
    });
    mapMarkersRef.current = [];

    // IMPORTANT: By default, show ALL markers (only filter if filters are explicitly set)
    // This ensures all places are shown by default as requested
    const filteredMarkers = filters && (filters.maxPrice || filters.minPrice || filters.amenities?.length) 
      ? markers.filter((property) => isPropertyMatching(property, filters))
      : markers; // Show all if no filters

    console.log(`📊 Markers to display: ${filteredMarkers.length} / ${markers.length}`, {
      showingAll: !filters || (!filters.maxPrice && !filters.minPrice && !filters.amenities?.length),
      filtered: filteredMarkers.map(p => ({ name: p.name, price: p.price })),
      excluded: filters && (filters.maxPrice || filters.minPrice || filters.amenities?.length)
        ? markers.filter(p => !isPropertyMatching(p, filters)).map(p => ({ 
            name: p.name, 
            price: p.price, 
            reason: filters?.maxPrice && p.price > filters.maxPrice ? `Price $${p.price} > maxPrice $${filters.maxPrice}` : 'Other filter'
          }))
        : []
    });

    // Verify all properties have valid coordinates before displaying
    const validMarkers = filteredMarkers.filter((property) => {
      const isValid = 
        property.latitude != null &&
        property.longitude != null &&
        !isNaN(property.latitude) &&
        !isNaN(property.longitude) &&
        property.latitude >= 39.94 &&
        property.latitude <= 39.96 &&
        property.longitude >= -75.22 &&
        property.longitude <= -75.15;
      
      if (!isValid) {
        console.warn(`⚠️ Invalid coordinates for ${property.name}:`, {
          lat: property.latitude,
          lng: property.longitude,
          address: property.address
        });
      }
      
      return isValid;
    });

    if (validMarkers.length !== filteredMarkers.length) {
      console.warn(`⚠️ Filtered out ${filteredMarkers.length - validMarkers.length} properties with invalid coordinates`);
    }

    // Add new markers with smart styling
    validMarkers.forEach((property) => {
      const style = getMarkerStyle(property, filters);
      
      const marker = new google.maps.Marker({
        position: { lat: property.latitude, lng: property.longitude },
        map,
        title: `${property.name} - $${property.price}/person`,
        label: {
          text: style.label,
          color: '#fff',
          fontSize: '12px',
          fontWeight: 'bold',
        },
        icon: {
          path: google.maps.SymbolPath.CIRCLE,
          scale: 14, // Larger for better visibility
          fillColor: style.color,
          fillOpacity: 1,
          strokeColor: '#fff',
          strokeWeight: 3,
        },
        animation: google.maps.Animation.DROP,
        zIndex: property.id && filters?.highlightedIds?.includes(property.id) ? 1000 : 100, // Highlighted markers on top
      });

      // Add enhanced info window
      const infoWindow = new google.maps.InfoWindow({
        content: createInfoWindowContent(property),
      });

      marker.addListener('click', () => {
        // Close all other info windows
        mapMarkersRef.current.forEach((m) => {
          const infoWindow = (m as any).infoWindow;
          if (infoWindow) {
            infoWindow.close();
          }
        });
        
        infoWindow.open(map, marker);
        (marker as any).infoWindow = infoWindow;
        console.log('📍 Marker clicked:', property.name);
      });

      mapMarkersRef.current.push(marker);
    });

    console.log(`✅ Added ${mapMarkersRef.current.length} markers to map`);

    // Fit bounds to show all markers if there are any
    if (validMarkers.length > 0) {
      const bounds = new google.maps.LatLngBounds();
      validMarkers.forEach((property) => {
        bounds.extend({
          lat: property.latitude,
          lng: property.longitude,
        });
      });
      
      // Add padding to bounds for better visibility
      map.fitBounds(bounds, {
        top: 50,
        right: 50,
        bottom: 50,
        left: 50,
      });
      
      console.log('🗺️ Map bounds updated to fit all markers');
    } else {
      // If no markers, reset to default center
      map.setCenter(center);
      map.setZoom(zoom);
      console.log('🗺️ No valid markers, resetting to default center');
    }
  }, [map, markers, filters, center, zoom]);

  // Show error message if map failed to load
  if (mapError) {
    return (
      <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-stone-50 to-stone-100">
        <div className="text-center p-8 max-w-md">
          <div className="text-4xl mb-4">🗺️</div>
          <h3 className="text-xl font-semibold text-stone-800 mb-2">
            Google Maps Not Configured
          </h3>
          <p className="text-stone-600 mb-4">
            {mapError === 'Google Maps API Key not configured. Please check .env.local file.'
              ? 'Please configure Google Maps API Key in .env.local file'
              : mapError}
          </p>
        </div>
      </div>
    );
  }

  return (
    <div
      ref={mapRef}
      className="w-full h-full"
      style={{ minHeight: '100vh' }}
    />
  );
}
